...
#porównanie czy hasło wprowadzone a hasło hashowane jest takie samo
...
def hash_password_with_salt(password: str) -> str:
    ...
    hashed_passowrd = hashlib.sha256(new_password.encode('ascii')).hexdigest()

    combined_salt_and_password = ''.join([hashed_passowrd, hashed_salt])
